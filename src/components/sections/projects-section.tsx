...
export function ProjectsSection() {
  return (
    <section id="projects" className="w-full py-16 md:py-24 lg:py-32 bg-[#030014] relative z-10 border-t border-cyan-900/20">
      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        <div className="mb-16 text-center">
          <h2 className="font-headline text-3xl font-bold tracking-[0.2em] uppercase text-cyan-400 mb-2 drop-shadow-[0_0_10px_rgba(6,182,212,0.5)]">
            Project Database
          </h2>
          <div className="h-1 w-24 bg-gradient-to-r from-transparent via-cyan-500 to-transparent mx-auto mb-6"></div>
          ...
          {projects.map((project, index) => (
            <div key={project.id} className="group relative flex flex-col items-center">
              ...
              <div className="mb-8 w-full max-w-[500px] relative z-20 group/image">
                <StarBorder as="div" color="#06b6d4" speed="5s" className="w-full h-full">
              ...
              <div className="w-full max-w-[500px] space-y-4 bg-white/5 border border-white/5 hover:border-cyan-500/30 p-6 transition-all duration-300 relative overflow-hidden group-hover:bg-white/[0.07]">
                ...
                <div className="flex items-center justify-between">
                  <h3 className="text-2xl font-bold font-headline text-white tracking-wide group-hover:text-cyan-400 transition-colors">
                    {project.title}
                  </h3>
                  <span className="font-mono text-xs text-cyan-500/50">ID: 00{index + 1}</span>
                </div>

                <p className="text-gray-400 text-sm leading-relaxed">
                  {project.description}
                </p>
                <div className="bg-black/40 p-3 border-l-2 border-cyan-500/30">
                  <ul className="text-gray-500 text-xs font-mono space-y-1">
                    {project.details.map((detail, idx) => (
                      <li key={idx} className="flex items-start">
                        <span className="text-cyan-500 mr-2">{'>'}</span> {detail}
                      </li>
                    ))}
                  </ul>
                </div>
                ...
                <div className="flex flex-wrap gap-2 pt-4 border-t border-white/5">
                  {project.tags.map(tag => (
                    <Badge key={tag} variant="outline" className="font-mono text-xs border-cyan-900/50 text-cyan-400/80 bg-cyan-950/10 hover:bg-cyan-500/20 hover:text-cyan-300 transition-colors">
                      {tag}
                    </Badge>
                  ))}
                </div>
